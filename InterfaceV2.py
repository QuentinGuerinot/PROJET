...
def get_entries():
    ...
    #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    cell.Tbmin = config.Cell.Tbmin
    cell.Tbmax= config.Cell.Tbmax
    cell.Cp= config.Cell.Cb;#Cb=Cp ???
    cell.h= config.Cell.h
...
def plot_optres(selgraph):
    ...
    if selgraph == 0:
        fig.add_subplot(111).plot(optres.t, optres.P,'black')
        plt.xlabel('Time [min]')
        plt.ylabel('puissance')
        plt.title('puissance')
    elif selgraph == 1:
        fig.add_subplot(111).plot(optres.t, optres.X,'red')
        plt.xlabel('Time [min]')
        plt.ylabel('soc')
        plt.title('soc')
    elif selgraph == 2:
